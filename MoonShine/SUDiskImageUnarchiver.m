...
- (void)extractDMGWithPassword:(NSString *)password
{
	...
	NSLog(@"Extracting %@ as a DMG", archivePath);
	...
	if (taskResult != 0)
	{
        ...
        if (password != nil && [resultStr rangeOfString:@"Authentication error"].location != NSNotFound && [delegate respondsToSelector:@selector(unarchiver:requiresPasswordReturnedViaInvocation:)]) {
            ...
            NSLog( @"hdiutil failed with code: %ld data: <<%@>>", (long)taskResult, resultStr );
            goto reportError;
        }
	}
	...
	if (floor(NSAppKitVersionNumber) > NSAppKitVersionNumber10_6) {
        ...
        if (error)
        {
            NSLog(@"Couldn't enumerate contents of archive mounted at %@: %@", mountPoint, error);
            goto reportError;
        }
...
reportError:
	...
	if (mountedSuccessfully)
		...
		NSLog(@"Can't mount DMG %@",archivePath);
	[pool drain];
}
